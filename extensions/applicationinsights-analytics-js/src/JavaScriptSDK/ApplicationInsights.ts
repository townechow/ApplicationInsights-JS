...
let _stringToBoolOrDefault = Util.stringToBoolOrDefault;
...
export class ApplicationInsights extends BaseTelemetryPlugin implements IAppInsights, IAppInsightsInternal {
    ...
    public static getDefaultConfig(config?: IConfig): IConfig {
        ...
        config.disableExceptionTracking = _stringToBoolOrDefault(config.disableExceptionTracking);
        config.autoTrackPageVisitTime = _stringToBoolOrDefault(config.autoTrackPageVisitTime);
        config.overridePageViewDuration = _stringToBoolOrDefault(config.overridePageViewDuration);
        config.enableUnhandledPromiseRejectionTracking = _stringToBoolOrDefault(config.enableUnhandledPromiseRejectionTracking);
    
        if (isNaN(config.samplingPercentage) || config.samplingPercentage <= 0 || config.samplingPercentage >= 100) {
            config.samplingPercentage = 100;
        }
    
        config.isCookieUseDisabled = _stringToBoolOrDefault(config.isCookieUseDisabled);
        config.isStorageUseDisabled = _stringToBoolOrDefault(config.isStorageUseDisabled);
        config.isBrowserLinkTrackingEnabled = _stringToBoolOrDefault(config.isBrowserLinkTrackingEnabled);
        config.enableAutoRouteTracking = _stringToBoolOrDefault(config.enableAutoRouteTracking);
            ...
            */
            ...
            _self.initialize = (config: IConfiguration & IConfig, core: IAppInsightsCore, extensions: IPlugin[], pluginChain?:ITelemetryPluginChain) => {
                ...
                const configGetters: ITelemetryConfig = {
                    ...
                    cookieDomain: () => _self.config.cookieDomain || config.cookieDomain,
                    ...
                    if (_window.addEventListener) {
                        ...
                        _window.addEventListener(_self.config.namePrefix + "locationchange", () => {
                            if (_properties && _properties.context && _properties.context.telemetryTrace) {
                                _properties.context.telemetryTrace.traceID = Util.generateW3CId();
